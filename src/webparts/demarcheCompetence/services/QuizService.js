export class QuizService {
     ...
     */
    async loadIntroductionQuestions() {
        try {
     ...
     * Load saved progress
     */
    async loadProgress(userId, quizType) {
        try {
     ...
     */
    async clearProgress(userId, quizType) {
        try {
            const progressKey = `quiz_progress_${userId}_${quizType}`;
     ...
     * Get quiz statistics
     */
    async getQuizStatistics(quizType) {
        try {
     ...
     * Export quiz results
     */
    async exportResults(quizType, format = 'csv') {
        try {
            const results = await this.sharePointService.getQuizResults();
     ...
     */
    transformIntroductionItem(item) {
        return {
            id: item.Id.toString(),
            title: item.Title,
    ...
    getTotalQuestions(responses) {
        if (Array.isArray(responses)) {
            return responses.length;
        }
        return 0;
    }
    getCorrectAnswers(responses) {
        if (Array.isArray(responses)) {
    ...
    async exportToCSV(data, quizType) {
        const headers = [
            'ID',
            'Utilisateur',
            'Email',
    ...
    async exportToJSON(data, quizType) {
        const exportData = {
