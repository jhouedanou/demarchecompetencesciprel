export class UserService {
     ...
     */
    async updateCompetenceLevel(competenceArea, newLevel, assessmentDate) {
        try {
            const currentUser = await this.getCurrentUser();
            ...
            if (existing) {
                ...
                const newProgress = Object.assign(Object.assign({ CompetenceArea: competenceArea }, updateData), { TargetLevel: 5, NextAssessment: this.calculateNextAssessment(assessmentDate || new Date()).toISOString() });
                await this.sharePointService.saveUserProgress(newProgress);
            }
        }
        catch (error) {
     ...
     */
    async setCompetenceTargets(targets) {
        try {
            const currentUser = await this.getCurrentUser();
            const userProgress = await this.sharePointService.getUserProgress(currentUser.email);
            const updates = [];
     ...
     */
    async getCompetenceAreasWithProgress() {
        try {
            const currentUser = await this.getCurrentUser();
            ...
            return competenceAreas.map(area => {
                ...
                return Object.assign(Object.assign({}, area), { userProgress: progress });
            });
        }
        catch (error) {
     ...
     */
    async scheduleNextAssessment(competenceArea, assessmentDate) {
        try {
            const currentUser = await this.getCurrentUser();
            const userProgress = await this.sharePointService.getUserProgress(currentUser.email);
            const existing = userProgress.find(p => p.CompetenceArea === competenceArea);
    ...
    calculateProgress(currentLevel, targetLevel) {
        if (targetLevel === 0)
            return 0;
        return Math.min(Math.round((currentLevel / targetLevel) * 100), 100);
    }
    calculateNextAssessment(lastAssessment) {
